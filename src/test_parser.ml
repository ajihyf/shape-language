...
let test_cases = [
  ...
  ("let x = a : array[byte] in let y = b : int | b > 0 in f(x, y)",
  ...
  ("fun(x : some[a] a, y : int -> int, z : (int -> int) | z(0) != 1) : some[a] array[a] -> 1",
  ...
  ("{s1, s2}",
   OK (SShape([SVar "s1"; SVar "s2"], false)));
   ("{$s1, s2}",
   OK (SShape([SVar "s1"; SVar "s2"], true)));
